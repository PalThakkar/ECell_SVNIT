      ...
      <div className="max-w-screen-xl mx-auto my-4 p-6 md:p-10 bg-black rounded-4xl shadow-lg" style={{ height: 'calc(100vh - 5.32cm)', overflowY: 'auto' }}>
        <div className="space-y-6">
          {faqData.map((faq, index) => (
            <div
              key={index}
              className={`relative p-6 rounded-lg transition-all duration-300 ease-in-out cursor-pointer ${
            ...
            >
              <div className="relative flex justify-between items-center">
                <h3 className={`text-xl font-semibold text-white transition-transform duration-300 ease-in-out transform ${openIndex === index ? 'translate-x-3' : ''}`}>
                  {faq.question}
                </h3>
              ...
              {openIndex === index && <p className="mt-2 text-neutral-300">{faq.answer}</p>} {/* Adjusted text color for better readability */}
